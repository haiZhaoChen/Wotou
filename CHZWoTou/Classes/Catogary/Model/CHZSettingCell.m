 ...
 *  提醒数字
 */
@property (strong, nonatomic) CHZBadgeButton *badgeButton;
@end

@implementation CHZSettingCell

- (CHZBadgeButton *)badgeButton
{
    if (_badgeButton == nil) {
        _badgeButton = [[CHZBadgeButton alloc] init];
    }
    return _badgeButton;
}
...
{
    // 存储数据
    CHZSettingSwitchItem *switchItem = (CHZSettingSwitchItem *)self.item;
...
+ (instancetype)cellWithTableView:(UITableView *)tableView
{
    ...
    CHZSettingCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[CHZSettingCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:ID];
...
- (void)setItem:(CHZSettingItem *)item
{
    _item = item;
...
- (void)setupData
{
    ...
    if (self.item.icon) {
        if ([self.item.flagUrl isEqualToString:@"http"]) {
            
            [self.imageView setImageWithURL:[NSURL URLWithString: self.item.icon] placeholderImage:[UIImage imageNamed:@"avatar_default_big"] options:SDWebImageRefreshCached];
        }else{
...
- (void)layoutSubviews
{
    ...
    dict[NSFontAttributeName] = self.subtitleLabel.font;
    CGSize subtitleSize = [self.item.subtitle sizeWithAttributes:dict];
...
- (void)setupRightView
{
    ...
    else if ([self.item isKindOfClass:[CHZSettingLabelItem class]])
    { // 右边是文字
        self.accessoryView = nil;
        CHZSettingLabelItem *labelItem = (CHZSettingLabelItem *)self.item;
        self.detailTextLabel.text = labelItem.text;
    }
    else if ([self.item isKindOfClass:[CHZSettingSwitchItem class]])
    { // 右边是开关
        CHZSettingSwitchItem *switchItem = (CHZSettingSwitchItem *)self.item;
        self.switchView.on = switchItem.isOn;
        self.accessoryView = self.switchView;
    }
    else if ([self.item isKindOfClass:[CHZSettingArrowItem class]])
    { // 右边是箭头
        self.accessoryView = self.arrowView;
    }
    else if ([self.item isKindOfClass:[CHZSettingCheckItem class]])
    { // 右边是打钩
        CHZSettingCheckItem *checkItem = (CHZSettingCheckItem *)self.item;
...
- (void)setIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.bg.image = [UIImage resizedImage:bgName];
