...
@interface CHZPickView()<UIPickerViewDelegate, UIPickerViewDataSource>
...
- (void)initView {
    ...
    _addressLb.text = [NSString stringWithFormat:@"%@",
                       _xianArr.count > 0? _xianArr[0][@"aname"]:@"未选择"];
}
...
- (void)initData {
    ...
    for (int i = 0; i < _shiArray.count; i++) {
        ...
        if ([_shiArray[i][@"pid"] integerValue] == index) {
    ...
    for (int i = 0; i < _xianArray.count; i++) {
        ...
        if ([_xianArray[i][@"city"] integerValue] == index) {
...
//    NSInteger shiRow   = [_pickerView selectedRowInComponent:1];
    ...
    _addressLb.text = [NSString stringWithFormat:@"%@",_xianArr.count > 0? _xianArr[xianRow][@"aname"]:@"未选择"];
    ;
}
...
//    NSInteger shiRow   = [_pickerView selectedRowInComponent:1];
    ...
    if (self.btnActionBlock) {
        self.btnActionBlock(_xianArr.count > 0? [_xianArr[xianRow][@"id"] integerValue]:0
                                ,_xianArr.count > 0? _xianArr[xianRow][@"aname"]:@"");
    }
